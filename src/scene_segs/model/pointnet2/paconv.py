...
import copy
import os
from src.scene_seg.model.pointnet2.utils import Laplacian
from src.scene_seg.util.paconv_util import weight_init, assign_score, get_ed, assign_kernel_withoutk
...
class PAConvGraph(PAConv):
    # 超图的超图卷积，超图拉普拉斯
    def __init__(self, input_dim, output_dim, bn, activation, config=None):
        super().__init__()
        # self.score_input = config.get('score_input', 'identity')
        # self.score_norm = config.get('score_norm', 'softmax')
        # self.temp = config.get('temp', 1)
    ...
    def forward(self, args):
        ...
        scores = self.scorenet(xyz, score_norm=self.score_norm)  # b,n,k,m
        HW = torch.matmul(in_feat.permute(0, 2, 3, 1), self.weightbank).view(B, N1, K, self.m, -1)  # b,n1,k,m,cout
        SHW = assign_score(score=scores, point_input=HW)  # b,n,k,o1,
        out_feat = out_feat.permute(0, 3, 1, 2)  # b,o1,n,k
        n, X = H.shape[0], HW.cpu().detach().numpy()
        structure = dict()
        # for i in range(nag[1].edge_index.shape[1]):
        #     structure[i] = [nag[1].edge_index[0,i], nag[1].edge_index[1,i]]

        adj = Laplacian(n, structure, X, m)
        ASHW = torch.spmm(adj, SHW)
        if self.bn is not None:
            out_feat = self.bn(out_feat)
        if self.activation is not None:
            out_feat = self.activation(out_feat)
