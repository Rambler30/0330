...
from src.nn import Stage, PointStage, DownNFuseStage, UpNFuseStage, \
    BatchNorm, CatFusion, MLP, LayerNorm, ConvStage, PaconvDownNFuseStage, \
    PaconvUpNFuseStage
...
class SPT(nn.Module):
    ...
    def __init__(
        ...
        if num_down > 0:
            ...
            # self.paconv_down_stages = nn.ModuleList([
                # DownNFuseStage(
                #     dim,
                #     h_edge_mlp[:-1],
    ...
    def forward(self, nag, batch_idx):
        ...
        self.npoint = nag[0].pos.shape[0]
        each_batch_len = self.npoint // 4
        new_xyz_idx = random.sample(range(self.npoint), 
                                                each_batch_len*4) 
        nag[0].x, norm_index, nag[0].y,\
        nag[0].super_index, nag[0].batch, nag[0].pos =\
        nag[0].x[new_xyz_idx], nag[0].norm_index(mode=self.norm_mode)[new_xyz_idx],\
            nag[0].y[new_xyz_idx], nag[0].super_index[new_xyz_idx],\
            nag[0].batch[new_xyz_idx], nag[0].pos[new_xyz_idx]
        """
        ...
        if self.down_stages is not None:
            ...
            for i_stage, (stage,node_mlp, h_edge_mlp, v_edge_mlp) in enum:
                ...
                # paconv_x, _ = self._forward_down_stage(paconv_stage, nag, i_level, x)
                # x = mlp_x + paconv_x
                down_outputs.append(x)
