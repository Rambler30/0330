...
class PointSegmentationModule(LightningModule):
    ...
    def model_step(self, batch, batch_idx):
        ...
        if isinstance(batch, NAG):
            logits, preds, y_hist = self.step_single_run_inference(batch, batch_idx)
        else:
    ...
    def step_single_run_inference(self, nag, batch_idx):
        """Single-run inference
        """
        y_hist = self.step_get_y_hist(nag)
        logits = self.forward(nag, batch_idx)
        if self.multi_stage_loss:
