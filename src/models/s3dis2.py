...
__all__ = ['Stage', 'DownNFuseStage', 'UpNFuseStage', 'PointStage', 'ConvStage', 'PaconvDownNFuseStage']
...
class PaconvDownNFuseStage(Stage):
    def __init__(self, *args, pool='max', fusion='cat', **kwargs):
        super().__init__(*args, **kwargs)
        ...
        # Fusion operator
        self.fusion = fusion_factory(fusion)
        in_mlp, mlp_activation, mlp_norm, mlp_drop = kwargs['in_mlp'],\
                                            kwargs['mlp_activation'], kwargs['mlp_norm'], kwargs['mlp_drop']
        self.conv = GraphConv(
            in_mlp, 
            activation=mlp_activation, 
            norm=mlp_norm, 
    ...
    def forward(
        ...
        if self.conv is not None:
            x = self.conv(x.T.unsqueeze(0), grouped_xyz, grouped_idx)
            x = x.squeeze(0).T
