...
class GDUT(BaseDataset):
    ...
    def all_base_cloud_ids(self):   
        return {
            'train': [f'{name}_{i}' for i in range(7) if i not in  self.fold for name in ['split', 'perturbate', 'shift']],
            'val': [f'{name}_{i}' for i in range(7) if i not in  self.fold for name in ['split', 'perturbate', 'shift']],
