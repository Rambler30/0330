...
class BaseDataset(InMemoryDataset):
    ...
    def __init__(
        ...
        super().__init__(root, transform, pre_transform, pre_filter)  # pre_transform保存到硬盘前进行变换
    ...
    def _process_single_cloud(self, cloud_path):
        ...
        #     num_points_to_keep = int(num_points * 0.80)
        #     random_indices = np.random.choice(num_points, num_points_to_keep, replace=False)
        #     data.y, data.pos, data.rgb, data.is_val, data.pos_room = data.y[random_indices], \
        #                                     data.pos[random_indices], data.rgb[random_indices], \
        #                                     data.is_val[random_indices], data.pos_room[random_indices]
