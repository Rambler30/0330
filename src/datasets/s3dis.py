...
def read_s3dis_area(
    ...
    """
    # List the object-wise annotation files in the room 房间内标注了的房间路径
...
def read_s3dis_room(
        ...
        xyz_room=False, align=False, is_val=True, verbose=False) -> Data :
    ...
    for i_object, path in enumerate(objects):
        ...
        if t:
            ...
            num_points_to_keep = int(num_points * 0.70)
...
def main(cfg):
    ...
    root = "/mnt/mountA/cwy/pointcloud/superpoint_transformer-master/data"
    stage='train'
    transform=None
    ...
    s3dis =  S3DIS(root=root, fold=5,transform=datamodule_cfg,save_y_to_csr=save_y_to_csr,in_memory=in_memory,
                  pre_transform=pre_transform,point_no_save_keys=point_no_save_keys, on_device_transform=on_device_transform)
    print("S3DIS Class the frist one block is :{}".format(s3dis.__getitem__(0)[0]))
    print("???")
